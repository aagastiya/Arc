...
function buildUserMessage(articles: ArticleWithSource[]): string {
  if (articles.length === 1) {
    ...
    // Publication date is deliberately withheld: the model kept restating it as
    // the date of the events, which no source text supports.
...
${article.resolved.text}`;
  ...
  return articles
    .map((article, index) => {
      ...
      return `Article ${index + 1} — Source: ${source}
Title: ${article.title}
Source quality: ${article.resolved.quality}
